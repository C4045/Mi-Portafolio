...
export class QuotationRepository {
  async findAll({ companyId, page, limit, skip, search, sortBy, sortOrder, status, customerId, dateFrom, dateTo }) {
    const where = { companyId, deletedAt: null, documentType: 'quotation' };
  ...
  async findById(id) {
    return prisma.sale.findFirst({
      where: { id, documentType: 'quotation', deletedAt: null },
      include: {
  ...
  async getLastDocumentNumber(companyId, serie) {
    return prisma.sale.findFirst({
      where: { companyId, documentSerie: serie, documentType: 'quotation', deletedAt: null },
