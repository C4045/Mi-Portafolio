...
export function SalesPage() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['sales', page, search, sortBy, sortOrder, filters],
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['sales'] }); setDeleteConfirm(null); },
  });
  ...
  const columns = [
    {
      ...
      render: (val, row) => (
        <div>
          ...
          <p className="text-xs text-muted-foreground">{formatDate(row.issueDate)}</p>
        </div>
      ),
    },
    {
  ...
  return (
    <div className="space-y-4 animate-in fade-in">
      <PageHeader title="Ventas" description="Gestiona las ventas y facturación"
        actionLabel="Nueva Venta" actionIcon onAction={() => navigate('/sales/new')} />
      ...
      <ConfirmDialog open={deleteConfirm !== null} title="Cancelar Venta" message={`¿Estás seguro de cancelar la venta ${deleteConfirm?.documentSerie}-${deleteConfirm?.documentNumber}?`}
        confirmLabel="Cancelar Venta" onConfirm={() => deleteMutation.mutate(deleteConfirm.id)} onCancel={() => setDeleteConfirm(null)} loading={deleteMutation.isPending} />
    </div>
  );
}
