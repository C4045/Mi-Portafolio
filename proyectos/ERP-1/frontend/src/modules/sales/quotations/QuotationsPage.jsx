...
export function QuotationsPage() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['quotations', page, search, sortBy, sortOrder, filters],
    queryFn: () => api.get('/quotations', { params: { page, limit: 20, search, sortBy, sortOrder, ...filters } }).then(r => r.data),
  });

  const deleteMutation = useMutation({
    mutationFn: (id) => api.delete(`/quotations/${id}`),
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['quotations'] }); setDeleteConfirm(null); },
  });

  const columns = [
    { key: 'documentNumber', label: 'N° Cotización', sortable: true, render: (val, row) => (<div><p className="font-medium">{row.documentSerie}-{val}</p><p className="text-xs text-muted-foreground">{formatDate(row.issueDate)}</p></div>) },
    ...
    { key: 'actions', label: '', render: () => null, actions: (row) => (
      <div className="flex items-center justify-end gap-1">
        <Link to={`/sales/quotations/${row.id}`}><Button variant="ghost" size="icon" className="h-8 w-8" title="Ver"><svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/><circle cx="12" cy="12" r="3"/></svg></Button></Link>
        {row.status === 'draft' && hasPermission('sales.update') && (
          <><Link to={`/sales/quotations/${row.id}/edit`}><Button variant="ghost" size="icon" className="h-8 w-8" title="Editar"><svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M17 3a2.85 2.83 0 1 1 4 4L7.5 20.5 2 22l1.5-5.5Z"/></svg></Button></Link>
  ...
  return (
    <div className="space-y-4 animate-in fade-in">
      <PageHeader title="Cotizaciones" description="Presupuestos y cotizaciones a clientes"
        actionLabel="Nueva Cotización" actionIcon onAction={() => navigate('/sales/quotations/new')} />
      <div className="flex flex-wrap gap-2 items-center">
        <select className="h-8 rounded-md border border-input bg-transparent px-2 text-xs" value={filters.status} onChange={(e) => { setFilters({ ...filters, status: e.target.value }); setPage(1); }}>
          ...
          <option value="draft">Borrador</option><option value="sent">Enviada</option><option value="accepted">Aceptada</option>
          <option value="rejected">Rechazada</option><option value="expired">Vencida</option><option value="converted">Convertida</option><option value="cancelled">Cancelada</option>
        </select>
      ...
      <ConfirmDialog open={deleteConfirm !== null} title="Cancelar Cotización" message={`¿Cancelar cotización ${deleteConfirm?.documentSerie}-${deleteConfirm?.documentNumber}?`}
