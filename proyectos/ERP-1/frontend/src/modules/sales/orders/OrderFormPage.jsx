...
export function OrderFormPage() {
  ...
  const isEditing = !!id;
  const [form, setForm] = useState({ customerId: '', issueDate: new Date().toISOString().split('T')[0], dueDate: '', notes: '' });
  const [items, setItems] = useState([]);

  const { data: customersData } = useQuery({ queryKey: ['customers-list'], queryFn: () => api.get('/customers/list').then(r => r.data) });
  const { data: productsData } = useQuery({ queryKey: ['products-select', 'all'], queryFn: () => api.get('/products', { params: { limit: 200, isActive: true } }).then(r => r.data) });
  const { data: orderData } = useQuery({ queryKey: ['order', id], queryFn: () => api.get(`/orders/${id}`).then(r => r.data), enabled: !!id });

  const createMutation = useMutation({
    mutationFn: (data) => isEditing ? api.put(`/orders/${id}`, data) : api.post('/orders', data),
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['orders'] }); navigate('/sales/orders'); },
  });

  const confirmMutation = useMutation({
    mutationFn: () => api.post(`/orders/${id}/confirm`),
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['orders'] }); queryClient.invalidateQueries({ queryKey: ['order', id] }); },
  });

  const fulfillMutation = useMutation({
    mutationFn: () => api.post(`/orders/${id}/fulfill`),
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['orders'] }); queryClient.invalidateQueries({ queryKey: ['order', id] }); },
  });
  ...
  useEffect(() => {
    if (orderData?.data) {
      ...
      setForm({ customerId: o.customerId, issueDate: o.issueDate?.split('T')[0] || '', dueDate: o.dueDate?.split('T')[0] || '', notes: o.notes || '' });
      setItems(o.items.map((i) => ({ id: i.id, productId: i.productId, productSku: i.product?.sku || '', productName: i.product?.name || '', quantity: i.quantity, unitPrice: i.unitPrice, discountRate: i.discountRate || 0, taxRate: i.taxRate || 10 })));
    }
  }, [orderData]);
  ...
  const updateItem = (idx, field, value) => {
    ...
    if (field === 'productId') { const prod = products.find((p) => p.id === value); if (prod) { newItems[idx].productSku = prod.sku; newItems[idx].productName = prod.name; if (!newItems[idx].unitPrice) newItems[idx].unitPrice = prod.salePrice; } }
    setItems(newItems);
  };
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    createMutation.mutate({ customerId: form.customerId, issueDate: form.issueDate, dueDate: form.dueDate||null, notes: form.notes, items: items.map((item) => ({productId: item.productId, quantity: Number(item.quantity), unitPrice: Number(item.unitPrice), discountRate: Number(item.discountRate), taxRate: Number(item.taxRate)})) });
  };

  const isView = isEditing && !['draft'].includes(orderData?.data?.status);

  return (
    <div className="space-y-4 animate-in fade-in">
      <div className="flex items-center justify-between">
        <div><h2 className="text-2xl font-bold tracking-tight">{isView ? 'Detalle Pedido' : isEditing ? 'Editar Pedido' : 'Nuevo Pedido'}</h2>
          <p className="text-sm text-muted-foreground">{orderData?.data?.documentSerie}-{orderData?.data?.documentNumber}</p>
        </div>
        <div className="flex gap-2">
          {isView && orderData?.data?.status === 'draft' && <Button onClick={() => confirmMutation.mutate()}>Confirmar Pedido</Button>}
          {isView && orderData?.data?.status === 'confirmed' && <Button onClick={() => fulfillMutation.mutate()}>Cumplir Pedido</Button>}
          {isView && <Button variant="outline" onClick={() => { api.post(`/orders/${id}/pdf`, {}, {responseType: 'blob'}).then(r => { const u=URL.createObjectURL(new Blob([r.data])); const a=document.createElement('a'); a.href=u; a.download=`PED-${id}.pdf`; a.click(); URL.revokeObjectURL(u); }); }}>PDF</Button>}
          <Button variant="outline" onClick={() => navigate('/sales/orders')}>Volver</Button>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit}>
        <Card><CardContent className="p-5 space-y-4">
          <div className="grid grid-cols-3 gap-4">
            <div className="space-y-2"><Label>Cliente *</Label>
              {isView ? <p className="text-sm font-medium pt-1">{orderData?.data?.customer?.businessName}</p> : (
            ...
            <div className="space-y-2"><Label>Fecha Entrega</Label>
              {isView ? <p className="text-sm pt-1">{form.dueDate || '—'}</p> : <Input type="date" value={form.dueDate} onChange={(e) => setForm({...form, dueDate: e.target.value})} />}
            </div>
          </div>
        ...
        <Card className="mt-4"><CardContent className="p-5">
          ...
          {items.length === 0 ? <p className="text-sm text-muted-foreground text-center py-8">Agregá productos al pedido</p> : (
        ...
        {!isView && (<div className="flex justify-end gap-2 mt-4">
          <Button type="button" variant="outline" onClick={() => navigate('/sales/orders')}>Cancelar</Button>
          <Button type="submit" disabled={createMutation.isPending || items.length===0}>{createMutation.isPending ? 'Guardando...' : isEditing ? 'Actualizar' : 'Crear Pedido'}</Button>
        </div>)}
      </form>
    </div>
  );
